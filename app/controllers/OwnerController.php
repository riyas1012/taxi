...
class OwnerController extends BaseController
{
	...
	public function get_braintree_token()
	{
		...
		if ($validator->fails())
		{
		   ...
		   $response_array = array('success' => false , 'error' => 'Klaidingas įvedimas' , 'error_code' => 401);
		   $response_code = 200;
		}
		else
		{
			...
			if( $owner_data = $this->getOwnerData($owner_id,$token,$is_admin)){
				...
				if($is_admin)
				{
					$response_array = array('success' => false , 'error' => 'Tokio vartotojo nėra' , 'error_code' => 410);
	...
	public function register()
	{
		...
		if ($password != "" and $social_unique_id == "") {
			...
			$response_array = array('success' => false, 'error' => 'Klaidingas įvedimas - either social_unique_id or password should be passed', 'error_code' => 401);
			$response_code = 200;
			goto response;
		}
		...
		{
			$response_array = array('success' => false, 'error' => 'Klaidingas įvedimas Telefono numeris turi būti perduota', 'error_code' => 401);
			$response_code = 200;
			goto response;
		}
		if ($validator->fails()) 
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin)
				{
					...
					try
					{
						...
						if(Config::get('app.default_payment') == 'stripe')
						{
							...
							{
								$response_array = array('success' => false , 'error' => 'Klaida' , 'error_code' => 450);
								$response_code = 200;
							}
						}
						else{
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin)
				{
					if($payment = Payment::find($card_id))
					{
						if($payment->owner_id == $owner_id)
						{
							...
							$response_array = array('success' => false , 'error' => 'Klaida' , 'error_code' => 440);
							$response_code = 200;
						}
					}
					else{
						$response_array = array('success' => false , 'error' => 'Klaida' , 'error_code' => 441);
						$response_code = 200;
					}
		...
		if ($validator->fails()) {
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin)
				{
					...
					if($ledger_count > 0)
					{
						$response_array = array('success' => false, 'error' => 'Šis kodeksas jau ėmėsi kitam vartotojui');
					}
					else
					{
						$ledger = new Ledger;
		...
		else 
		{
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					...
					{
						$response_array = array('success' => false, 'error' => 'Šis vartotojas neturi kreipimosi kodą');
