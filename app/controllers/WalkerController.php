...
class WalkerController extends BaseController
{
	...
	public function register()
	{
		...
		elseif ($social_unique_id != "" and $password != "") {
			$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 401);
			$response_code = 200;
			goto response;
		}
		...
		if ($validator->fails()) 
		{
			...
			$error_messages = $validator->messages()->all();
			$response_array = array('success' => false, 'error' => 'Klaidingas įvedimas', 'error_code' => 401, 'error_messages' => $error_messages );
			$response_code = 200;
		} else {
			...
			if ($walker=Walker::where('phone', '=', $phone)->first()) 
			{
				...
				{
					$response_array = array('success' => false, 'error' => 'Telefonas jau užregistruotas', 'error_code' => 402);
					$response_code = 200;
				}
				else
				{
					...
					//$verification_id=$confirmation_number;

					$user_update=Walker::where('phone','=',$phone)->update(array('first_name'=>$first_name,'last_name'=>$last_name,'email'=>$email,
					'country_code'=>$country_code,'phone'=>$phone,'password'=>$passwords,'token'=>generate_token(),'token_expiry'=>generate_expiry(),
					'type'=>$type,'picture'=>$s3_url,'device_token'=>$device_token,'device_type'=>$device_type,'bio'=>$bio,'address'=>$address,'state'=>$state,
					'country'=>$country,'zipcode'=>$zipcode,'login_by'=>$login_by,'is_available'=>1,'is_active'=>0,'is_approved'=>0,
					'social_unique_id'=>$social_unique_id,'verification_id'=>$confirmation_number));

					
					// Send sms
					$settings = Settings::where('key','sms_verification_code')->first();
					$pattern = $settings->value;
					$pattern = str_replace('%id%', $confirmation_number, $pattern);
	...
	public function verification_code()
	{
		...
		if (Input::has('phone')) 
		{
			...
			if ($verification_code!= "" and $phone!= "")
			{
				...
				else
				{
					if ($walker = Walker::where('phone', '=', $phone)->first())
					{
						...
						{
							$response_array = array('success' => false, 'error' => 'Klaidingas patvirtinimo kodas', 'error_code' => 403);
							$response_code = 200;
						}
					}
					else
					{
						$response_array = array('success' => false, 'error' => 'Neregistruotas vartotojas', 'error_code' => 403);
						$response_code = 200;
					}
				}
			}
			...
			{
				$response_array = array('success' => false, 'error' => 'Klaidingas įvedimas', 'error_code' => 404);
				$response_code = 200;
			}
		}
	...
	public function login()
	{
		...
		if (Input::has('phone') && Input::has('password')) {
				...
				if ($walker = Walker::where('phone', '=', $phone)->first()) {
					if($walker->is_registered==1)
					{
						if (Hash::check($password, $walker->password)) {
							if ($login_by != "manual") {
								$response_array = array('success' => false, 'error' => 'Klaidingas prisijungimas', 'error_code' => 417);
								$response_code = 200;
							} else {
								if ($walker->device_type != $device_type) {
									$walker->device_type = $device_type;
							...
							$response_array = array('success' => false, 'error' => 'Klaidingas vardas arba sklaptažodis', 'error_code' => 403);
							$response_code = 200;
						}
					}
					else
					{
						...
						$pattern = str_replace('%id%', $walker->verification_id, $pattern);
						sms_notification($walker->id,'walker', $pattern);
						$response_array = array('success' => false, 'error' => 'Nepatvirtinta', 'error_code' => 404);
						$response_code = 200;
					}
					
				} else {
					$response_array = array('success' => false, 'error' => 'Neregistruotas vartotojas', 'error_code' => 404);
					$response_code = 200;
				}
			}
		} elseif (Input::has('social_unique_id')) {
			...
			if ($socialValidator->fails()) {
				...
				{
					$response_array = array('success' => false, 'error' => 'Klaida registracijoje', 'error_code' => 404);
					$response_code = 200;
				}
			}
		}
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->is_dog_rated == 0) {
									...
									$response_array = array('success' => false, 'error' => 'Jau įvertinta', 'error_code' => 409);
									$response_code = 200;
								}
							} else {
								$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 407);
								$response_code = 200;
							}
						} else {
							$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 408);
							$response_code = 200;
						}
					} else {
						$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 405);
						$response_code = 200;
					}
				} else {
					if ($is_admin) {
						$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 410);

					} else {
						$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 406);
						...
						if ($walk = Walk::find($walk_id)) {
							if ($walk->walker_id == $walker_id) {
								...
								if ($walk->is_walk_started == 0) {
									...
									$response_array = array('success' => false, 'error' => 'Kelionė jau prasidėjo', 'error_code' => 416);
									$response_code = 200;
								}
							} else {
								$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 407);
					...
					if ($request = Requests::find($request_id) )
					{
						if($request->current_walker == $walker_id)
						{
							if($accepted == 1)
							{
								...
								$title_lithuanian="Surasta Taksi";
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->confirmed_walker != 0) {
									...
									$title_lithuanian="Taksi pradėjo važiuoti";
									send_notifications($request->owner_id,"owner",$title,$title_lithuanian,$message);
									...
									$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 413);
									$response_code = 200;
								}
							} else {
								$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 407);
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->is_walker_started == 1) 
								{
									...
									$response_array = array('success' => false, 'error' => 'Kelionė dar neprasidėjo', 'error_code' => 413);
									$response_code = 200;
								}
							} else {
								$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 407);
					...
					if (is_token_active($walker_data->token_expiry) || $is_admin)
					{
						if ($request = Requests::find($request_id))
						{
							if ($request->confirmed_walker == $walker_id)
							{
								...
								{
									$response_array = array('success' => false, 'error' => 'Taksi dar neatvyko', 'error_code' => 413);
									$response_code = 200;
								}
							}
							else 
						...
						if ($request = Requests::find($request_id))
						{
							if ($request->confirmed_walker == $walker_id)
							{
								if ($request->is_walker_arrived == 1)
								{
									...
									$title_lithuanian="Vairuotojo Atsauktas";
									$message = $response_array;
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->is_walker_arrived == 1) {
									...
									$title = "Trip Started";
									$title_lithuanian="Kelionė prasidėjo";
									$message = $response_array;
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->is_started == 1) 
								{
									...
									$title_lithuanian="Jusu Kelione Bus Baigta";
									$message = $response_array;
									send_notifications($request->owner_id,"owner",$title,$title_lithuanian,$message);
						...
						if ($request = Requests::find($request_id)) {
							if ($request->confirmed_walker == $walker_id) {
								...
								if ($request->is_started == 1) {
									...
									$response_array = array('success' => false, 'error' => 'Kelionė dar neprasidėjo', 'error_code' => 414);
									$response_code = 200;
								}
							} else {
								$response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 407);
