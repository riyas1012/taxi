                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($owner_data->token_expiry) || $is_admin) {
                        ...
                        if ($dog) {
                            ...
                            $response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 445);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Klaida', 'error_code' => 405);
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->owner_id == $owner_data->id) {
                                if ($request->is_completed == 1) {
                                    ...
                                    $response_array = array('success' => false, 'error' => 'Pasivaikščiojimas nėra baigtas', 'error_code' => 409);
                                    $response_code = 200;
                                }
                            } else {
        ...
        if ($validator->fails()) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if($typestring==''){
                        $response_array = array('success' => false, 'error' => 'Nesurastas teikėjas.', 'error_code' => 405);
                        $response_code = 200;
